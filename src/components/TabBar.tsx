...
export function TabBar () {
  ...
  return (
    <nav class='fixed bottom-0 flex items-center justify-around h-18 w-full border-t backdrop-blur-2xl bg-base/80 border-neutral-700'>
      { barLinks.map(({ name, path: link, Icon: LinkIcon }) => (
        <a
          key={uuidv4()}
          href={link}
          class={`${path === link ? 'active' : ''} group flex flex-col items-center justify-center gap-1 h-full aspect-square`}
        >
          <div class='flex items-center justify-center h-fit w-fit p-1 px-4 rounded-full text-neutral-300 group-[.active]:bg-neutral-700'>
            <Icon class='size-6'>
