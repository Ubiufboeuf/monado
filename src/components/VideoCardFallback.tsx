export function VideoCardFallback ({ class: className }: { class?: string }) {
  return (
    <article class={`${className} h-full w-full flex flex-col items-center`}>
      <div class='w-full xs:not-sm:w-[max(80%,var(--breakpoint-xs))] sm:not-md:w-[min(100%,500px)]'>
        <section class='w-full aspect-video bg-neutral-700 xs:rounded-xl' />
        <section class='w-full flex h-20 xs:h-24 pt-3 gap-2 flex-1'>
